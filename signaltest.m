...
num = 16 ;%一个周期内的采样点数，由于三个数联合判决因此注意后面乘以3
fs=num/Tp;%单位Hz采样率恰好是周期倒数的整数倍
nbei=1;%s最大速率的nbei分之一（即一比特占用几个周期）
...
s1=[sin(2*pi/Tp/fs*n).*am;zeros(1,1)];%补一个零为了用于匹配fft变换
...
% s1=sin(2*pi/Tp/fs*repmat(n,1,8)+xiangwei);%不画图就不再补0了
...
finaldata2=finaldata0((round(order/2/num/3)+1)*num*3+1:(3*N1*num));%去除之前的人为添加的一个零值
finalimput2=jumps(round(order/2/num/3)+1:3*N1);
%num*3为一组数，即48个
% for lie=1:8
% figure(2*lie-1)
% subplot(2,2,1)
